...
- `window.location` / `about:srcdoc` を基準にリソース URL を実行時計算するページ（例: 一部の Swagger UI 構成で OpenAPI 定義を `new URL("...", window.location.href)` で読み込むケース）は、リモートデータの読み込みに失敗することがあります。
