...
- 総括セクションは「総合判断」「指摘内訳 (インライン指摘のラベル別件数、優先度順に件数>0 のもののみ。0 件なら `指摘なし`)」「良かった点 1〜2」を簡潔にまとめる。
...
- `FAILURE` のジョブがあれば上記手順でログ詳細まで読み、関連箇所への `[must]` の根拠として使う。
  ...
  - 失敗の中身が **フォーマッタ / Linter レベル** (例: `prefer-const` のみ) のときは、ノイズ抑制ルールに従いインライン指摘は出さず、総括 (総合判断) での 1 文言及に留める。
